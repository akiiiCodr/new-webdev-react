...
function ToastNotification({ message, onClose, type, index }) {
  ...
  const getToastStyle = () => {
    switch (type) {
      case 'success':
        return { backgroundColor: 'rgba(255, 255, 255, 0.2)', color: '#fff' }; // Green for success
      case 'error':
        return { backgroundColor: 'rgba(255, 255, 255, 0.2)', color: '#fff' }; // Red for error
      case 'warning':
        return { backgroundColor: 'rgba(255, 255, 255, 0.2)', color: '#fff' }; // Orange for warning
      default:
